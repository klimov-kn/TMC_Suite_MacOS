...
CTMCGROUTView::CTMCGROUTView()
{
	...
	strcpy( gr_typX[0].pszXname, "Time n" );
	...
	strcpy( gr_typY[4].pszYname, "Phase in radian" );
	gr_typY[5].nType = TMC_GROTS_TYPE_PHAS_G;
	strcpy( gr_typY[5].pszYname, "Phase in gradus" );
...
void CTMCGROUTView::PrepareDoubleGraph()
{
	...
	int i, j, ii, iph;
	double r11, r12, r13, r14, x12, dT, freq, s11;
	...
	if(pDoc->grdoc.psGraph[i].OutFlag)
	{
		for( j = 0, ii = 0, r11 = 0, r12 = 0, r13 = 0, r14 = 0, iph = 0; j < tmcgrwin.psGraph[i].nPoint; j++)
		{
			...
			case TMC_GROTS_TYPE_ps:
				tmcgrwin.psGraph[i].pPoint[j].x = pDoc->grdoc.psGraph[i].pFreq[j]*(1.e+12);
				;
				break;
			...
			case TMC_GROTS_TYPE_LDB:
				if(pDoc->grdoc.psGraph[i].pcExpr == NULL )
				{
					if( r14 < DBL_MIN ) tmcgrwin.psGraph[i].pPoint[j].y = 0.;
					else tmcgrwin.psGraph[i].pPoint[j].y = -20.*log10( r14 );
					if( j == 2 )
					{
						r11 = (double)( pDoc->grdoc.psGraph[i].pSmatr[0].x );
						r12 = (double)( pDoc->grdoc.psGraph[i].pSmatr[1].x );
			...
			case TMC_GROTS_TYPE_PHAS_R:
				if(pDoc->grdoc.psGraph[i].pcExpr == NULL )
				{
					if( r14 > (2*PI) ) r14 -= (2*PI);
					if( r14 > (2*PI) ) r14 -= (2*PI);
					if( r14 <= 0. )   r14 += (2*PI);
					if( r14 <= 0. )   r14 += (2*PI);
					if( r14 >= (2*PI*0.999) ) r14 -= (2*PI);
					...
					if( j > 2 )
					{
						...
						if( (r13 < 0.0)&&(r12 > 0.0) )
						{
							freq = pDoc->grdoc.psGraph[i].dFreq;
							x12 = pDoc->grdoc.psGraph[i].pFreq[j-1];
							dT = pDoc->grdoc.psGraph[i].pFreq[j]-x12;
							iph = (int(freq*x12));
							r14 = ( x12 - (iph+0.5)/(freq) + r12*dT/(r12-r13) )*(2*PI*freq);
						};
					};
				}
				else
				...
				if(pDoc->grdoc.psGraph[i].pcExpr == NULL )
				{
					...
					if( j > 2 )
					{
						...
						if( (r13 < 0.0)&&(r12 > 0.0) )
						{
							...
							dT = pDoc->grdoc.psGraph[i].pFreq[j]-x12;
							iph = (int(freq*x12));
							r14 = ( x12 - (iph+0.5)/(freq) + r12*dT/(r12-r13) )*(360*freq);
						};
					};
				}
				else
...
void CTMCGROUTView::OnDrawGraph3(CDC *pDC, int i)
{
	...
	{
	   text.Format("- %s - %d falling"  ,	pDoc->grdoc.psGraph[i].szGrapPodp,
										pDoc->grdoc.psGraph[i].nInp1);
	}
	else
	{
	   text.Format("- %s - %d scattering"  ,	pDoc->grdoc.psGraph[i].szGrapPodp,
										pDoc->grdoc.psGraph[i].nInp1);
	};
...
void CTMCGROUTView::OnViewTranslateChangexmaxxminEnd() 
{
	...
	if( tmcgrwin.nXType == TMC_GROTS_TYPE_nT )
	{
		for( i = 0; i < pDoc->grdoc.nGraph; i++ )
		{
			if( pDoc->grdoc.psGraph[i].OutFlag )
			{
				...
				{
					j = 1;
					dXMax = pDoc->grdoc.psGraph[i].nPoint-1;
				}
				else
				{
					if( (pDoc->grdoc.psGraph[i].nPoint-1) > dXMax )
					{
						dXMax = pDoc->grdoc.psGraph[i].nPoint-1;
					};
				};
