...
CTMCGROUTDoc::CTMCGROUTDoc()
{
	...
	grdoc.psGraph  = NULL;
	error[0] = '\0';
	grdoc.PointDrawFlag = TRUE;
	grdoc.nXType  = TMC_GR_TYPE_FGHz;
	grdoc.nYType  = TMC_GR_TYPE_WSVR;
	grdoc.Xmin = 1.0;
	grdoc.Xmax = 10.0;
	grdoc.Ymin = 1.0;
	grdoc.Ymax = 1.2;
...
BOOL CTMCGROUTDoc::DocFileNewData(int n) //allocation memory for new data and set standart data
{
	...
	grdoc.psGraph = new	TMC_GR_DOC1[ n ];
	if( grdoc.psGraph == NULL )
	{
...
BOOL CTMCGROUTDoc::WriteDocFile()
{
 ...
 for( i = 0; i < 16; i++) sGrDoc[i].PointColor = scPointColor;
 ...
 if(sprintf(ch,"%s %lg\n%s %s\n%s %d; %d;\n%s %d\n%s %d\n%s %lg\n%s %lg\n%s %lg\n%s %lg\n%s %d\n%s %d\n%s %s\n%s %s;%ld;%ld;%ld;%ld;%ld;%d;%d;%d;%d;\n%s %d;\n%s %d;\n%s %d;\n%s %d;\n%s %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d;\n",
			TMC_GROUT_DOCFILE_ID_dXAxiesUnit, dXAxiesUserUnit,
			TMC_GROUT_DOCFILE_ID_dXAxiesName, csXAxiesUserName,
...
// save graphics default parameters 
 if(sprintf(ch,"%s %lg%s %s%s %d; %d;%s %d%s %d%s %lg%s %lg%s %lg%s %lg%s %d%s %d%s %s%s %s;%ld;%ld;%ld;%ld;%ld;%d;%d;%d;%d;%s %d;%s %d;%s %d;%s %d;%s %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d;",
			TMC_GROUT_DOCFILE_ID_dXAxiesUnit, dXAxiesUserUnit,
			TMC_GROUT_DOCFILE_ID_dXAxiesName, csXAxiesUserName,
			TMC_GROUT_DOCFILE_ID_PointDrawFlag , grdoc.PointDrawFlag, bLossPoint,
			TMC_GROUT_DOCFILE_ID_nXType,	 grdoc.nXType,
...
void CTMCGROUTDoc::MakeDocFileName( void )
{
 ...
 int i;
	...
	{					  // set File.dat as title
		// FALSE: не добавлять во MRU — GetTitle() возвращает заголовок ("TMCGROUT1"),
		// а не полный путь; иначе CRecentFileList->Add бросает CInvalidArgException.
	...
	if( !IsDocFileName( ch ) )	 
	{
		...
		// FALSE: не добавлять рабочий путь во MRU (пользователь открыл .s/.soc).
...
BOOL CTMCGROUTDoc::IsDocFileName( char *ch)
{
	...
	fclose(fd);
	if( strncmp( ch1, TMC_GROUT_DOCFILE_ID, strlen(TMC_GROUT_DOCFILE_ID) ) == 0 ) return TRUE;
	else
		return FALSE;
...
void CTMCGROUTDoc::ReadGraph1( int nGr, char *szFileName, int *nPoint, double **pFreq, _complex **pSmatr )
{
 ...
 ReadInputModeFromFileName( csFileName, szFileName, inp1, mod1, inp2, mod2 );
 if( (inp1 < 0)||(mod1 < 0)||(inp2 < 0)||(mod2 < 0) )
 {
	 inp1 = grdoc.psGraph[nGr].nInp1;
	 mod1 = grdoc.psGraph[nGr].nMod1;
	 inp2 = grdoc.psGraph[nGr].nInp2;
	 mod2 = grdoc.psGraph[nGr].nMod2;
 };
 strcpy( grdoc.psGraph[nGr].szFileName + n + 1, csFileName);

 read_S_matrix_element( &inp1,
						&mod1,
						&inp2,
						&mod2,
 ...
 if( *nPoint > 0 )
 {
	 grdoc.psGraph[nGr].nInp1 = inp1;
	 grdoc.psGraph[nGr].nMod1 = mod1;
	 grdoc.psGraph[nGr].nInp2 = inp2;
	 grdoc.psGraph[nGr].nMod2 = mod2;
//	 sprintf( grdoc.psGraph[nGr].szGrapPodp, "nPoint=%d", *nPoint );
 };
...
void CTMCGROUTDoc::SetOneReadGraphParameters(char *ch)
{
	 ...
	 if( ch[0] == '#' ) 
	 {
		 ...
		 if( strncmp( ch, TMC_GROUT_DOCFILE_ID_dXAxiesName, strlen(TMC_GROUT_DOCFILE_ID_dXAxiesName) ) == 0 )
		 {
			 csXAxiesUserName.Format( "%s", ch + strlen(TMC_GROUT_DOCFILE_ID_dXAxiesName) );
		 };
		 if( strncmp( ch, TMC_GROUT_DOCFILE_ID_dXAxiesUnit, strlen(TMC_GROUT_DOCFILE_ID_dXAxiesUnit) ) == 0 )
		 {
			 sscanf( ch + strlen(TMC_GROUT_DOCFILE_ID_dXAxiesUnit), "%lg",  &(dXAxiesUserUnit) );
		 };
...
void CTMCGROUTDoc::ReadInputModeFromFileName(CString &csFileName, char *szFileName, int & inp1, int & mod1, int & inp2, int & mod2)
{
	int i;
	...
	inp2 = -1;
	mod2 = -1;
	csFileName.Format( "%s", szFileName );

	if( szFileName[0] != '*' ) return;

	for( i = 1; szFileName[i] != '*'; i++ )
	{
		if( szFileName[i] == '\0' ) return;
	};

	csFileName.Format( "%s", szFileName + i + 1 );
	szFileName[i] = '\0';
	if( sscanf( szFileName + 1, "%d;%d;%d;%d;", &inp1, &mod1, &inp2, &mod2 ) != 4 )
	{
		inp1 = -1;
		mod1 = -1;
		inp2 = -1;
