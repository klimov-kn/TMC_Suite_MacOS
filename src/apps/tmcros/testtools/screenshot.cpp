...
//
//   tmcros_shot <файл.png> [файл задания] [ширина] [высота]
...
int main(int argc, char *argv[])
{
    ...
    const QString out = args.size() > 1 ? args.at(1) : QStringLiteral("tmcros.png");
    ...
    // Необязательный пятый довод — команда перед снимком (число из resource.h
    // или afxres.h). Так проверяется, что пункт меню меняет картинку.
    const int command = args.size() > 5 ? args.at(5).toInt() : 0;
    if (command > 0) {
        for (QAction *action : window.findChildren<QAction *>()) {
            if (action->data().toInt() == command) { action->trigger(); break; }
        }
    }
